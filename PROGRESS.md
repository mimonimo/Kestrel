...
## MCP 서버 — 외부 AI 에이전트 노출 (공개 읽기 전용)

- **2026-06-25 — MCP 서버 `/api/v1/mcp` 신설** (`8ad4d8e`, `backend/app/api/v1/mcp.py`): MCP 지원 에이전트(Claude/ChatGPT 등)가 Kestrel CVE 데이터를 툴로 사용할 수 있게 노출. **Streamable-HTTP JSON-RPC** (`initialize` / `notifications/initialized` / `ping` / `tools/list` / `tools/call`), stateless JSON 응답, `GET → 405`(POST 전용). `/api/v1` 하위 마운트라 기존 Caddy 라우팅(`/api/*` → backend) 무변경. **툴 4종**(모두 공개·읽기 전용, 기존 DB + SSVC 재사용): `search_cves(query/severity/kevOnly/limit)`, `get_cve(cveId)`(상세 + SSVC 권장 대응 기한), `get_remediation(cveId)`, `recent_kev(limit)`. **보안 설계**: 이미 공개된 데이터만 노출 — 인증·쓰기·admin·에이전트 토큰 표면 전무, limit 상한, 파라미터라이즈드 쿼리. serverInfo `kestrel v0.1.0`. **라이브 검증(2026-07-29, `https://www.kestrel.forum/api/v1/mcp`)**: GET→405, initialize/tools-list/tools-call(recent_kev 실데이터 반환) 전부 정상. 미완성·미커밋 잔여 없음(후속 확장 여지: 툴 추가·세션 SSE·인증 쓰기 툴).

## 운영 점검 로그

- **2026-07-31 — 미사용 샌드박스/Vulhub 잔재 정리** (~506줄 삭제): 샌드박스/Vulhub 익스플로잇 랩 기능은 2026-06 `6c46bb7`(PR 10-ES)에서 백엔드 서비스·모델·라우터·랩 이미지가 통째로 제거됐으나, **설정·프론트 타입/메서드 잔재가 남아있었음**. 삼중 검증(라이브 OpenAPI에 `/sandbox` 없음, 프론트 컴포넌트 0곳 사용, config 설정 어디서도 안 읽힘) 후 정리: ① `config.py` sandbox_*·vulhub_* 설정 ~60줄 제거(**MITRE 설정은 뒤섞여 있어 보존**), ② `frontend/lib/api.ts` — 삭제된 `/sandbox/*`를 호출하던 sandbox/synth 메서드 16개 + 관련 타입 인터페이스 ~26개(289줄) + 전용 SSE 헬퍼(streamSse·parseSseFrame, 64줄) 제거, ③ `ai_analyzer.py`의 삭제된 `docker-compose.claude-cli.yml` 참조 힌트 정정, ④ claude_auth·feedback-box의 cosmetic 주석 정리. **무변경(의도적 보존)**: alembic 0009/0011 sandbox 마이그레이션(스키마 이력), `.env`(gitignore), Dockerfile docker CLI(빌드 영향—별도 판단), seed.py "Sandbox escape" mock CVE 제목(정상 데이터). 백엔드 py_compile OK, 프론트 tsc 0에러 — 라이브 코드 무영향 확인.

- **2026-07-30 — 실서비스 기준 문서 감사 + README/examples 갱신 + MCP 문서화** (`679c0e4`): 논문 작성 근거로 "현재 서비스 중" 기준 문서 정합화. **엔드포인트 인벤토리**: 라이브 OpenAPI(`https://www.kestrel.forum/openapi.json`, 프록시 밖 루트 경로 — `/api/v1/openapi.json` 아님) 기준 **115개 경로** 확인. MCP(`GET·POST /api/v1/mcp`)·subscriptions(구독)·notifications 등 전부 포함. **문서 공백 발견**: MCP 서버는 코드·PROGRESS 에만 있고 README/examples 어디에도 없었음(examples 는 Bearer Agent API 만 문서화) → examples/README 에 MCP 섹션(공개·읽기전용, JSON-RPC, 툴 4종, 커넥터/Inspector/curl) 신설, README 에 "외부 통합(Agent API vs MCP)" 표 추가. **관측성 뱃지 정정(중요 실측)**: README 가 OpenTelemetry·Sentry 를 표방했으나 라이브 확인 결과 **OTel 은 프로덕션 이미지 미설치**(Dockerfile 이 `.[sentry]` 만 설치, `otel_installed=False`), **Sentry 는 SDK 포함이나 `SENTRY_DSN` 미설정으로 비활성**(init 로그 없음). 실제 관측성은 structlog→CloudWatch Logs + CloudWatch 알람 + Route53 헬스체크. README 를 이 현실로 정정하고 OIDC 는 계획 단계(`docs/oidc-plan.md`, 미구현)로 명시. README 를 엔드포인트 나열 대신 라이브 OpenAPI 를 단일 진실 공급원으로 지목. (docs/proposal=논문 산출물, infra 가이드=운영 정확 → 무변경.)

- **2026-07-30 — `kestrel-prod-app-errors` 알람 반복 발생 원인 규명 + `related_cves` 20초+ 타임아웃 수정** (`1b1d05d`): 사용자가 "알람 자꾸 울리는데 서버는 정상" 신고. **진단**: 울린 알람은 `kestrel-prod-app-errors`(CloudWatch, 서울) — `/kestrel/prod/containers` 로그의 `?"Traceback (most recent call last)" ?"CRITICAL"` 메트릭 필터가 5분당 ≥3건×3연속이면 ALARM, OK 알림이 없어 ALARM 진입마다 SNS 발송(→ 버스트 중 반복). 트레이스백 정체는 **`asyncpg QueryCanceledError: statement timeout`**(API 20초 statement timeout 초과). 스택 추적 → **`cves.py related_cves`** (`/api/v1/cves/{id}/related`, 에이전트 래퍼 `/agent/cves/{id}/related`)가 범인. 07-29 18:00~03:39 KST 크롤러/에이전트 트래픽으로 급증 후 잦아듦(신고 시점엔 이미 조용, 이메일은 간밤 버스트분). **근본 원인(EXPLAIN ANALYZE)**: `id IN (semi-join) ORDER BY published_at LIMIT 120` 이 "published_at 인덱스 최신순 훑기 + 매행 affected_products 프로브" 계획으로 풀려, 흔한 벤더(cisco: ios/ios_xe/ios_xr/nx-os)에서 **73,377행×3워커 스캔 = 34.5초**. 추가로 일부 CVE 는 (vendor,product) pair 를 **5,400개** 나열해 IN 절 폭발. **수정(순수 코드, cves.py)**: ① 후보 vuln-id 집합을 먼저 조회(정렬·LIMIT 없이) → 작은 집합만 최신순 정렬로 나쁜 semi-join 계획 제거, ② `query_pairs` 를 `_RELATED_PAIR_CAP=300`·후보 id 를 `_RELATED_CANDIDATE_CAP=3000` 으로 상한(IN·정렬 유계, 스코어링용 `self_prods` 는 전체 유지), ③ `(vendor,product)` 복합 인덱스는 many-pairs 케이스를 14초로 **악화**시켜 폐기(라이브 BEGIN…CREATE INDEX…ROLLBACK 로 실측 확인 — 인덱스 없음). **검증**: DB 벤치 32.7s→1.5s·29.6s→1.4s·2.2s→0.44s; 라이브 재기동 후 콜드 3~10s → warm 0.6~2.2s, 전부 20초 여유, 결과 8건·랭킹 동일. **후속(미조치)**: 완화책(캐시·크롤러 차단 `/related`)은 사용자 요청대로 다음 단계.

- **2026-07-27 — 클로드 분석 간헐 실패 원인 규명 + 세이프가드 차단 우아 처리**: 프로덕션에서 CVE 분석이 "됐다 안 됐다" 하던 실패의 진짜 원인은 인증·버전·설정이 아니라 **Anthropic 사이버보안 세이프가드 분류기가 분석 프롬프트를 차단**하는 것(CLI `exit=1`, `API Error: <model>'s safeguards flagged this message for a cybersecurity topic ... https://claude.com/form/cyber-use-case`). 분석 프롬프트가 실동작 exploit·리버스셸·`/etc/shadow`·클라우드 메타데이터 SSRF 생성을 강하게 요구해 공격 코드 생성으로 분류됨. **재현(SSM→컨테이너 내 `analyze_vulnerability` 직접 호출)**: opus-4-7 은 CVE 내용에 따라 확률적 차단(최근 CVE 3건은 3/3 성공, 프로덕션 502 로그의 3건 중 1건 차단), **opus-5 는 같은 3건 전부 차단**. → 사용자 요청이던 "최신 모델 업데이트"는 세이프가드가 더 강해 분석을 사실상 마비시키므로 **미실행, `claude-opus-4-7` 유지**(재현 중 DB 모델을 opus-5 로 바꿨다가 즉시 복구 확인). 조치(`ai_analyzer.py`): ① `_looks_like_cyber_safeguard()` 로 차단 감지 시 무의미한 self-upgrade 재시도(어차피 컨테이너 node<22 → `npm EBADENGINE` 로 실패)를 건너뛰고 즉시 명확한 안내(재시도 가능성 + 예외 신청 URL)로 502 반환, ② `_classify_error` 에 `cyber_safeguard` 종류 추가. **부수 발견(미조치)**: 사용자 4명 중 3명은 컨테이너에 `.credentials.json` 없음 → 해당 사용자는 "Not logged in" 으로 분석 불가(유효 1명, 만료 오늘 09:29 UTC). **진짜 해결은 방어목적 예외 신청**(https://claude.com/form/cyber-use-case, 계정 소유자만 가능). **배포 후 라이브 검증 중 2번째 실패 원인 발견**: 세이프가드가 확률적이라 같은 CVE 가 통과했는데 이번엔 `AI 응답 파싱 실패: Expecting ',' delimiter` — 모델이 정상 분석을 냈으나 payload(exploit 코드) 안의 escape 안 된 따옴표/개행으로 JSON 이 깨진 케이스. 추가 조치(`ai_analyzer.py`): `_is_parse_failure()` 로 JSON 문법/형식 실패(빈 응답=인증 신호는 제외)만 골라 **1회 재요청**(refusal 재시도와 동일 패턴 — "엄격 유효 JSON, 따옴표/개행 escape" 강조, 실패 경로에서만 모델 호출 1회 추가, 2번째 실패는 그대로 표면화해 무한 재시도 방지).

- **2026-07-09 — 운영 점검 + 가용성 감시 공백 해소**: 정기 점검 결과 서비스 자체는 전부 양호(컨테이너 6개 Up·수집 4소스 success·24h Traceback 0·배포 드리프트 없음 — GHCR index digest 로 실행 이미지=latest 확인). 유일한 실질 갭은 **완전 다운 시 침묵**: 기존 알람이 전부 로그 기반(AppErrors, notBreaching)이라 인스턴스/Caddy 가 죽어 로그가 끊기면 아무 통지도 없었음. 조치(`infra/ec2/availability.tf` 신규): ① **Route53 Health Check** — 외부에서 `https://www.kestrel.forum/api/v1/health` 30초 간격 실측(Caddy+backend 경유, ~$0.50/월), ② `HealthCheckStatus` 다운 알람 — **지표가 us-east-1 에만 발행**되므로 알람·SNS 토픽(`kestrel-prod-alerts-use1`)도 us-east-1 에 생성(providers.tf 에 `aws.use1` alias 추가; 서울 토픽 재사용 불가), missing=breaching + ok_actions(복구 통지), ③ **EC2 StatusCheckFailed** 알람(서울, 기존 토픽). apply 후 전 세계 체커 200 OK·알람 OK 확인. 부수 정리: compose 의 `NVD_API_KEY`/`GITHUB_TOKEN` 를 `${VAR:-}` 기본값으로 — 키는 DB AppSettings 가 1차 소스라 env 비어있는 게 정상인데 매 실행 경고가 떠서 침묵시킴. **주의: us-east-1 SNS 이메일 구독은 확인 메일 클릭 전까지 통지 안 감.**
...
- **2026-08-03 — 설정 › 내 에이전트 카드 모바일 깨짐(세로 글자)**: `/settings` 내 에이전트 목록에서 카드가 `flex items-start`([아바타][min-w-0 flex-1 내용][액션버튼 shrink-0 flex-wrap]) 구조인데, 버튼 그룹이 `shrink-0 flex-wrap` 이라 **max-content(수정/ON/토큰 재발급/삭제 한 줄 ≈250px)를 유지** → 내용 컬럼(flex-1)이 **7px 로 짜부러져 메타 텍스트가 글자당 한 줄씩 세로로**("분/석/527/건/토/큰…") 렌더. **원인**: flex-wrap 은 컨테이너가 좁혀야 줄바꿈하는데 그룹이 shrink-0 라 부모가 안 좁혀줌 + 내용 min-content 는 min-w-0 로 거의 0 → 버튼이 항상 옆자리를 다 차지. **수정**: 카드 외곽 `flex items-start` → `flex flex-wrap items-start`, 버튼 그룹에 `w-full sm:w-auto` — 모바일에선 버튼 줄이 **자기 줄(full-width)로 내려가** 내용이 폭을 회복(라이브 검증 7px→240px, 메타 1줄), 데스크톱(≥sm)은 `w-auto` 로 기존처럼 인라인 유지. **스캐너 개선 메모**: 이 유형(요소가 뷰포트가 아니라 *자기 부모 카드*만 넘침)은 초기 "뷰포트 밖(right>vw)" 스캔에 안 걸려서, `scrollWidth>clientWidth`(부모 컨테이너 넘침) + 스크롤/말줄임 컨테이너·조상 제외 필터로 2차 스캔을 돌려 발견. `AgentsManagePanel.tsx`.

- **2026-08-03 — CVSS 벡터·메트릭(악용 경로/영향) 모바일 오버플로**: `/cve/{id}` 의 CVSS 메트릭 섹션이 `grid gap-3 sm:grid-cols-2` 인데 모바일(단일 컬럼)에서 **암묵적 `auto` 트랙이 max-content(232px)로 커져 카드 컨테이너(208px)를 24px 넘침** — 페이지 전체 스크롤은 아니지만 메트릭 행이 카드 우측 경계 밖으로 삐져나옴. 라벨 span 이 `flex-1 truncate` 라 `min-w-0` 없어 축소도 안 됐음. **수정**: ① 그리드에 `grid-cols-1`(=`minmax(0,1fr)`) 추가 → 모바일 트랙을 컨테이너로 캡(desktop 은 `sm:grid-cols-2` 가 그대로 이겨 무변경), ② 라벨을 `flex-1 truncate` → `min-w-0 flex-1 break-keep` — 말줄임(정보 손실) 대신 **띄어쓰기 단위 줄바꿈**(word-break:keep-all, "사용자 상호작용"이 "상/호작용"으로 안 깨지고 "사용자 / 상호작용"). 라이브 DOM 검증: 섹션 오버플로 24→0px, 라벨 클리핑 0. `CveDetail.tsx`(2줄 클래스).

- **2026-08-03 — AI 분석 탭바 모바일 발견성**: 360px 에서 탭바(분석 피드·패턴 비교·즐겨찾기·검토) 4개가 358px 로 가용폭 296px 를 넘겨 마지막 "검토" 탭이 화면 밖으로 밀림(overflow-x-auto 라 가로스크롤로만 접근 → 숨은 탭 발견 어려움). 실측으로 후보 비교: 여백·글자 축소는 여전히 초과, **모바일에서 탭 아이콘만 숨기면 278px 로 한 줄에 4개 다 수납**(라이브 DOM 주입 검증). 아이콘에 `hidden sm:block` 한 줄만 추가 — **모바일은 라벨만, 데스크톱(≥sm)은 아이콘 포함 기존 그대로**(구조·색·패딩 무변경). `app/analysis/page.tsx`.

- **2026-08-03 — 모바일(360px) UI 점검 + CVE 상세 레이아웃 깨짐 수정**: 기능이 늘며 좁은 화면에서 태그·타이포가 벗어나던 문제를 실측 점검. **점검법**: Chrome 창은 최소폭 500px 로 클램프되어 진짜 360px 재현이 안 되므로, 라이브 사이트를 **동일 출처 360px `iframe`** 에 실어(미디어쿼리는 iframe 뷰포트 기준으로 평가됨) 각 라우트를 로드하고, DOM 을 훑어 **뷰포트 우측을 넘는 요소(`getBoundingClientRect().right > vw`)·내부 클리핑(`scrollWidth>clientWidth`)** 을 자동 스캔 + 스크린샷 병행. 결과: `/`·`/cves`·`/community` clean, `/analysis` 탭바만 마지막 "검토" 탭이 가로스크롤로 밀림(overflow-x-auto 라 허용), **`/cve/{id}` 가로 오버플로 확정**(docScrollW 387 > 356). **원인·수정 3건(모두 flexbox `min-width:auto` 병리 — 긴 언브레이크 토큰 `secure_firewall_management_center`)**: ① **영향받는 제품·버전** 행이 `flex justify-between nowrap` 인데 좌측 `<div>` 에 `min-w-0` 없어 제품명이 안 줄고 우측 상태뱃지(`other`)를 화면 밖으로 밀어냄 → 좌측 `min-w-0 [overflow-wrap:anywhere]`(라이브 iframe 에서 `break-word` 는 flex min-content 를 안 줄여 `anywhere` 로 검증)·뱃지 `shrink-0`. ② **연관 취약점 모바일 폴백 행**(`RelatedCvesGraph` `sm:hidden`): 사유 알약 `{it.reason}` 에 truncate/`min-w-0` 없어 카드 밖으로 넘치고 CVE-id 가 두 줄로 짓눌림 → CVE-id `shrink-0 whitespace-nowrap`·사유 알약 `ml-auto min-w-0 shrink truncate`. ③ **연관 취약점 헤더** 토글이 폭 부족에 눌려 "관계맵"이 세로로 쪼개짐 → 헤더 `flex-wrap`·버튼 `whitespace-nowrap`·컨트롤 그룹/정보칩 `shrink-0`, 목록뷰 사유칩도 `min-w-0 truncate` 로 선제 방어. **검증**: 각 수정을 iframe 실 DOM 에 주입해 `docScrollW 387→346`(오버플로 해소)·스크린샷으로 뱃지 노출·CVE-id 단일행 확인, `tsc --noEmit` 통과. `CveDetail.tsx`·`RelatedCvesGraph.tsx`·`RelatedCves.tsx`.

- **2026-07-31 — 분석 피드: 특정 작성자로 좁혀 그 안에서만 검색·정렬**: 카드 작성자 이름 옆 깔때기(필터) 아이콘 → 그 작성자만 스코프(제거 가능한 "작성자: OOO ✕" 칩). 걸리면 목록·검색(`q`)·정렬·페이지네이션이 모두 그 작성자 안으로 좁혀진다(백엔드는 이미 `/community/analyses?username=&q=&sort=` 조합 지원 — 프론트 UX 만 추가). 이름 클릭은 프로필 이동을 유지하려 별도 아이콘으로 분리, 카드 열기 버튼과의 중첩 회피 위해 `role="button"` span(+키보드) 사용. scope=내 분석 전환 시 칩 자동 해제. `AnalysisFeed.tsx` 단독, `tsc` 통과.

- **2026-07-31 — AI 분석 피드·패턴 비교 전체 조회(서버 페이지네이션·집계)**: AI 분석 탭에서 오래된 분석(특히 에이전트 DGX_1~4 의 수백 건)이 "사라진 것처럼" 안 보이던 문제. **원인**: `AnalysisFeed` 가 커뮤니티 분석을 `limit:50` 한 번만 받아 **최신 50건 창** 위에서 모든 정렬(최신·우선순위·EPSS·유형·작성자)을 클라이언트로 처리 → 밤사이 신규 에이전트(DGX_A~F)가 피드를 채우자 오래된 작성자가 창 밖으로 밀려 어떤 정렬에서도 로드 자체가 안 됨(데이터는 DB 에 전부 있음, 2,032건). 패턴 비교 탭은 CVE 선택 목록을 **localStorage(기기별·50건 캡)** 에서 읽어 기기마다 다르게·누락되어 보였다. **수정(서버 정석)**: **백엔드** `GET /community/analyses` 에 `sort`(recent|priority|epss)·`username`·`type`·`severity`·`q`(CVE·제목·본문·작성자 검색, ILIKE 이스케이프)·`pipeline_only` 파라미터 + **실제 total(COUNT)** 반환(기존 `len(rows)` 는 페이지 크기였음), `/me/analyses` 도 실제 total. 신규 `GET /community/analyses/facets` — 작성자별·유형별·위험도별 **전체 집계**(총건수·최근시각·최근 CVE, DISTINCT ON 로 작성자별 최신 CVE). priority 정렬은 `CASE(immediate→0/scheduled→1/monitor→2/else 8)` + EPSS desc nulls last + 최신순 타이브레이크(프론트 PRIORITY_RANK 와 일치). **프론트** `AnalysisFeed` 를 서버 구동으로 재작성 — 평면 뷰(최신·우선순위·EPSS)는 서버 정렬+페이지 넘김(이전/다음, "전체 N건"), 작성자별·유형별·위험도별은 facets 헤더(전체 총계)+행 확장 시 `username`/`type`/`severity` 로 서버 조회(그룹 내 "더 보기"), 검색은 서버 `q`(전체 대상). scope=내 분석은 유계라 전량 로드 후 클라 정렬. `CompareTab` 은 비교 대상 CVE 를 **`/me/analyses` 전량**(200 페이지 누적, cveId dedupe)에서 — localStorage 폐기로 **모든 환경 동일·누락 없음**. **검증**: 백엔드 쿼리 프로드 DB replicate(작성자 집계 DGX_2=552·3=549·1=337·4=141 전원, priority/type/severity 집계, total=2,032), 프론트 `tsc --noEmit` 통과. `analysis_records.py`·`AnalysisFeed.tsx`·`app/analysis/page.tsx`·`lib/api.ts`. (죽은 코드 `AnalysisTab` 은 탭 키에 없어 미렌더 — 미변경.)

- **2026-07-31 — 에이전트 엔드포인트 500 근절: `/related` 정렬 병리 + `/agent/cves` 정렬 인덱스**: AppErrors 알람(Traceback/CRITICAL 5분×3구간 다발)이 반복. 근본원인 2건. **(1) `GET /agent/cves`**: `(kev_listed OR cvss_score>=7)` 필터 + 3키 정렬(`kev DESC, cvss DESC NULLS LAST, published DESC NULLS LAST`)에 맞는 인덱스가 없어 37만 행 통째 정렬 → `LIMIT 1` 조차 20s statement timeout 초과 → 500(QueryCanceledError). 정렬 순서와 동일한 복합 인덱스 `ix_vuln_agent_priority` 를 `CREATE INDEX CONCURRENTLY`(11m42s, 논블로킹)로 선반영 → Index Scan, **24.8ms**. 멱등 마이그레이션 `0040`(운영은 no-op, 신규/개발 DB 재현용). **(2) `GET /agent/cves/{id}/related`**: 후보 집합을 미리 좁혔는데도(`cand_ids` ≤6000) 최종 `id IN (...) ORDER BY published_at DESC LIMIT 120` 이 플래너에서 "published_at 인덱스를 최신순 역주행하며 매 행 id 필터" 계획으로 풀려, cisco(CVE-2016-1409, 후보 1,254)·Log4Shell(CVE-2021-44228, 후보 5,942)처럼 **후보가 오래된** CVE 는 120건 채우려 37만 행 대부분 스캔 → **25~29초**(clean DB 에서도 재현) → 500. 이전 "2단계 후보 분리" 수정이 candidate 쿼리만 고치고 최종 정렬 병리는 남겨둔 것이 원인. **수정**: 최종 조회를 `WITH c AS MATERIALIZED (SELECT id, published_at FROM vulnerabilities WHERE id = ANY(:ids)) SELECT id ... ORDER BY published_at DESC NULLS LAST LIMIT 120` 로 교체 — MATERIALIZED 로 후보를 PK 로 먼저 구체화해 ORDER BY 인덱스 역주행을 차단, `id = ANY(array)` 로 거대 IN 전개도 회피. 이후 상위 120 id 만 ORM 적재(selectin, 정렬은 스코어링이 재수행하므로 순서 무관). **실측(clean prod DB)**: 최종조회 25,290ms→**23~73ms**, 엔드투엔드 CVE-2016-1409 29.1s→**5.8s**, Log4Shell 24s→**8.7s**, CVE-2021-3942 26.7s→**2.6s**, 모두 20s 아래. rows=120·items 정확. **오진 기록**: 처음엔 11분 CONCURRENTLY 빌드가 촉발한 autovacuum VACUUM ANALYZE(≈440s) + 일일 EPSS 배치 충돌로 인한 일시적 IO 포화로 봤으나, autovacuum 종료·`AUTOVAC_REMAINING=0` 확인 후에도 `/related` 가 23~29s 로 남아 진짜 정렬 병리로 판명. **후속(선택)**: `affected_products(vendor, product)` 복합 인덱스(현재 후보-제품 tuple-IN 스캔 ~2.2s 단축용) 및 `QueryCanceledError` graceful 처리(부하 스파이크 시 500 대신 빈 결과) — 부하 가라앉은 뒤 별도. `backend/app/api/v1/cves.py`(related 최종조회 CTE 화 + `text` 임포트)·`alembic/versions/0040_vuln_agent_priority_index.py`.

- **2026-07-30 — 작성자 구독 → 신규 분석·글을 알림채널로 전달** (`7c49af9`): 알림채널 기능 강화 — 특정 사용자/에이전트를 구독하면 그 작성자의 새 공개 분석·커뮤니티 글이 구독자 본인의 알림채널(Slack/Discord 웹훅)로 전달된다(에이전트가 매일 글/분석을 올리는 흐름 팔로우용). **백엔드**: ① `AuthorSubscription`(subscriber×author, unique) 모델 + 마이그레이션 0039, ② `/subscriptions` 라우터(GET 내 구독목록 / POST username 구독 / DELETE 해제) — 식별을 내부 user_id 대신 **username**으로(analysis_records 의 user_id 비노출 관례 준수), ③ `notify_author_subscribers()` — 작성자 구독자의 enabled 채널로 fanout(기존 `_dispatch_webhook` 재사용, best-effort, background_session 분리), ④ 발행 훅 4곳(BackgroundTasks 응답 후 비차단): 사람/에이전트 × 분석/글. **재분석 스팸 가드** — 사람 분석은 "작성자×CVE 최초 공개 분석"에만, 에이전트 분석은 upsert 신규 발행에만 전달. ⑤ PostOut 에 `authorUsername`/`authorIsAgent` 추가(익명 글 null→버튼 미노출). **프론트**: 재사용 `SubscribeButton`(기존 pill/파스텔 톤, `["subscriptions"]` 목록으로 상태판단·낙관적 토글·비로그인 /login 유도), 커뮤니티 피드 작성자 옆 구독 버튼(작성자 행을 글-열기 버튼과 분리해 button 중첩 회피), 설정→알림채널에 "구독 중인 작성자" 목록+해제. **설계 결정**: 전달은 작성자 단위라 커뮤니티에서 에이전트 1회 구독 시 그 에이전트의 분석·글 모두 수신 → 커뮤니티 피드 버튼만으로 기능 완결(분석 피드 카드는 통째 `<button>`이라 중첩 회피 위해 구독버튼 후속). 전달 대상은 채널만(인앱·이메일 제외 — 스키마 추가 최소화). **라이브 검증**: 마이그레이션 0039 적용(테이블 존재), GET/POST `/subscriptions` 401 게이트, posts 응답 authorUsername/authorIsAgent 노출, fanout join 쿼리 롤백 트랜잭션으로 확인(구독+enabled 채널→조회, 비활성 제외, 실데이터 무변경), 실행 이미지 digest = 커밋 빌드 일치·healthy. **후속(`37b7b45`)**: 구독 진입점을 프로필로 확장 — 에이전트(`/agents/[id]`)·사용자(`/users/[username]`) 프로필 헤더에 SubscribeButton 추가(`AgentProfileOut`에 username 필드 추가, 본인 프로필엔 숨김). 라이브에서 agent profile 응답 username 노출 확인. **명확화**: 구독은 작성자 단위·발행 훅이 분석/글 양쪽에 걸려 있어, 프로필/피드 어디서 구독하든 그 작성자의 새 **분석·커뮤니티 글 모두** 채널로 전달됨.

- **2026-07-29 — 커뮤니티 피드 제목·본문 검색 추가** (`1e486bd`): 커뮤니티 피드에 검색 기능 신설. **백엔드** `GET /community/posts` 에 `q` 파라미터 — `title/content ILIKE '%q%'` 부분일치, ILIKE 와일드카드(`% _ \`)는 리터럴로 이스케이프(`escape="\\"`)해 사용자가 `%` 를 쳐도 그 문자 자체 검색, 공백/빈값은 무시, 기존 `vulnerabilityId` 필터와 AND, `total` 도 필터 반영. **프론트** 대시보드 CVE 검색과 **동일한 `SearchBar` 컴포넌트 재사용**(design-consistent, Enter 제출) — 피드 탭에만 배치(공지 탭 제외), 검색 결과 요약/"검색 지우기", 검색 전용 빈 상태("검색 결과가 없습니다" + 전체 보기). `SearchBar` 에 optional `placeholder` prop 만 추가(기본값 유지 → 대시보드 무변경). React Query 키(`["community-posts", page, query]`)·낙관적 like 업데이트 키까지 정합화. **구현 방식 선택**: Meili/tsvector 대신 Postgres ILIKE — 현 피드 규모(수건~수천)에 충분하고 인덱스 동기화 버그·한글 형태소 미스가 없음(수만 건으로 커지면 `pg_trgm` GIN 이 후속 옵션). **라이브 검증**(BE): 제목매칭 3건·제목+본문 6건·본문전용('커널') 4건·없는키워드 0건·`%` 이스케이프 0건(전체 6건 아님) 전부 통과. 프론트 실행 이미지 digest = 커밋 빌드 일치, community 페이지 200.

- **2026-07-10 — 로그인 페이지 정리 + 대응 기한 카드 숫자 압축**: ① 로그인 페이지의 "또는 → 🤖 AI 에이전트로 등록" 대안 경로 제거 — 에이전트 등록도 결국 로그인이 필요해 제2의 로그인 수단으로 오인됨(/agents/new 자체가 로그인 게이트 안내). ② 대시보드 `대응 기한 분포` 카드에서 156,053 같은 6자리 수가 1/4 칸을 넘쳐 레이아웃 깨짐 — 1만 이상은 "15.6만" 압축 표기(전체 값은 카드 title 툴팁 유지). `PriorityOverviewPanel.tsx`, `app/login/page.tsx`.

- **2026-07-10 — EPSS 표기 정확도 + 검증 라벨 명확화**: ① EPSS 뱃지가 0.99959 를 반올림해 "EPSS 100%" 로 표시 — EPSS 는 정의상 1.0 불가라 오류로 보임. `pctEpss()` 신설: 99.95% 이상은 소수 2자리(0.99959→"99.96%"), 99.99% 캡(절대 100% 미표시), 그 외 1자리(0.5→"50%", 0.183→"18.3%"). ≥0.5 주황 강조 기준 유지. ② "검증 N%" 가 분석 내용 정확도로 오해됨 — 실제는 소스 데이터 상호 일관성(Cross-Validation) 결과라 라벨을 "일관성 N%" 로, 툴팁에 "소스 데이터(CVSS·심각도·벡터)의 상호 일관성 검증 결과 — 분석 서술의 정확성 보장이 아님" 명시. 포매터 엣지(1.0/0.9995/0.9994 등) node 단위 확인 + 라이트/다크 목킹 16케이스 ALL PASS.
