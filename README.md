...
- **백엔드 ground-truth 검증 (PR 9-L)** — 합성 lab 의 신뢰 게이트가 LLM 폐쇄 루프에서 백엔드 probe 로 이동. RCE / path-traversal / SSTI / XSS / time-based SQLi 5 클래스에 대해 백엔드가 직접 만든 페이로드 + `docker exec` 로 stamp 한 카나리 + benign nonce 음성 대조로 echo machine 형태 lab 을 빌드 직후에 거부. echo trap 셰이프(`success_indicator` 가 너무 짧음 / payload 와 동일 / `files[*].content` 에 그대로 박힘 / `response_kind` 누락) 는 빌드 전에 차단.
...
- **백엔드 ground-truth 검증 (PR 9-L)**: 빌드 직후 spawn 한 lab 에 대해 `response_kind` 별로 매칭되는 backend probe (RCE 카나리 read / path-traversal 카나리 read / SSTI 산술식 평가 / XSS 두-nonce reflect / time-based blind SQLi) 를 실행. 각 probe 는 페이로드/카나리/예상 substring/음성 대조를 모두 백엔드가 만들어내며, lab 안의 카나리는 `docker exec` 로 stamp 합니다 (HTTP 표면을 거치지 않으므로 LLM 이 위조 불가). 양성 probe 가 하나라도 통과 + 음성 대조 깨끗하면 `verified=backend_probe`, 모두 실패하면 `rejected` (legacy LLM-indicator 결과는 통과해도 구제하지 않음). probe 가 적용 안 되는 vuln class 는 `llm_indicator_only` 약식 검증으로 통과시키되 매핑 카드에 ⚠️ 표시.
...
- **Lab 의 "CVE 정확성" 은 backend probe 가 커버하는 vuln class 안에서만 보장됩니다**. PR 9-L 시점에 RCE / path-traversal / SSTI / XSS / time-based SQLi 5 종은 backend probe 가 ground-truth 로 거부/통과를 판정합니다. 그 외 클래스(XXE, SSRF, deserialization, auth bypass, IDOR 등) 는 약식 `llm_indicator_only` 검증으로만 통과되며 매핑 카드에 ⚠️ 표시 — 사용자 👍/👎 평가가 보조 신호로 사용됩니다.
...
- [ ] backend probe 클래스 확장 (XXE / SSRF / deserialization / auth bypass)
- [ ] 합성 lab 다중 후보 보존 + best-of-N 선택 (PR 9-M 예정)
