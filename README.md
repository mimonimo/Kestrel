...
- **백엔드 ground-truth 검증 (PR 9-L/9-M)** — 합성 lab 의 신뢰 게이트가 LLM 폐쇄 루프에서 백엔드 probe 로 이동. RCE / path-traversal / SSTI / XSS / time-based SQLi / XXE / open-redirect / deserialization 8 클래스에 대해 백엔드가 직접 만든 페이로드 + `docker exec` 로 stamp 한 카나리 (또는 redirect nonce / file-side-effect) + benign nonce 음성 대조로 echo machine 형태 lab 을 빌드 직후에 거부. echo trap 셰이프(`success_indicator` 가 너무 짧음 / payload 와 동일 / `files[*].content` 에 그대로 박힘 / `response_kind` 누락) 는 빌드 전에 차단.
