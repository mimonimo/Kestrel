...
_USER_TEMPLATE = (
    ...
    "payload_example (문자열, 그대로 복사해 실행 가능한 PoC):\n"
    "  형식은 상황에 맞게 직접 선택(curl, HTTP 원문, python requests, nuclei 템플릿, msfconsole 등).\n"
    "  반드시 여러 줄로 작성하세요 — 한 줄로 압축하지 말고, JSON 문자열 내 줄바꿈은 실제 개행(\\n)으로 표현합니다. "
    "명령 옵션, 헤더, 본문은 줄을 나눠 가독성 있게 배치하세요.\n"
    "  첫 줄: '# 용도: ...' — 이 PoC가 증명하는 바를 한 줄.\n"
    "  페이로드 내부에 '# 핵심: ...' 주석으로 어떤 부분이 취약 로직을 건드리는 결정적 문자열/값인지 1개 이상 지적.\n"
    "  치환값은 TARGET_HOST, ATTACKER_IP, SESSION_COOKIE 등 대문자 플레이스홀더.\n"
    "  끝 줄: '# 확인 포인트: ...' — 어떤 응답 코드/본문/로그를 보면 성공인지.\n"
    "  실제 타인 자산·개인정보 금지. 방어 목적 PoC 수준으로 작성.\n\n"
