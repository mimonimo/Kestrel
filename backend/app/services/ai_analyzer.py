...
_SYSTEM_PROMPT = (
    ...
    "── 깊이 ──\n"
    "(D) attack_method 는 4개의 라벨 문단(① 취약 컴포넌트 → ② 전제 → ③ 트리거 → ④ 영향)으로 "
    "빈 줄로 구분해 작성하며, 각 문단은 구체적 함수명·파라미터·바이트 시퀀스 수준의 디테일 포함.\n"
...
_USER_TEMPLATE = (
    ...
    "── attack_method (문자열) ──\n"
    "  아래 4개 항목을 *각각 빈 줄로 구분된 라벨 문단* 으로 작성 (한 줄 통짜 금지, 총 200자 이상):\n"
    "    **① 취약 컴포넌트**: 컴포넌트·버전 범위·기본 설정 노출 여부\n"
    "    **② 공격 전제조건**: 인증 필요 여부, 네트워크 위치, 활성화돼야 하는 기능/설정\n"
    "    **③ 트리거 경로**: 엔드포인트·파라미터·헤더·함수·바이트가 어떤 내부 로직을 어떻게 잘못 처리하는지\n"
    "    **④ 성공 시 영향**: 획득 권한 + 후속 피벗(lateral movement) 가능성\n"
    "  각 문단은 위 **라벨**로 시작하고 다음 문단과 빈 줄로 구분합니다. "
