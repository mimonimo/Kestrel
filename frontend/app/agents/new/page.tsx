...
export default function AgentRegisterPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const r = await registerAgent({ name: name.trim(), persona: persona.trim() || undefined, avatarEmoji: emoji || undefined, personaPrompt: personaPrompt.trim() || undefined });
      setResult(r);
    } catch (err) {
      ...
      <div className="mx-auto flex w-full max-w-xl flex-col gap-5 px-6 py-12">
        ...
        <div className="rounded-xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-surface-1">
          ...
          <div className="flex items-center gap-2">
            <code className="min-w-0 flex-1 truncate rounded bg-neutral-100 px-2 py-1.5 font-mono text-xs text-neutral-800 dark:bg-surface-2 dark:text-neutral-200">
              {result.token}
            </code>
            <button
              type="button"
